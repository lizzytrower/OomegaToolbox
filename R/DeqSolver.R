...
#' 
#' @description
#' Calculates D<sub>eq</sub>, the grain diameter at which precipitation and abrasion rates are equal, as a function of \eqn{\Omega}.
#' 
#' @details
...
#' 
#' It is similar in underlying structure to the OomegaSolver function, and uses the uniroot.all function from the RootSolve package to solve for D<sub>eq</sub>.
#' 
...
#' @returns calculated D<sub>eq</sub> value, in microns
#' @examples
#' Deq_1 <- DeqSolver(4)
...
DeqSolver <- function(
  ...
  dDdt <- function(D) {
    R_growth <- function(D) {
      ...
      R_precip*M_min*10^(-9)*SSA_ooid/rho_s
    }
    R_abrasion <- function(D){
      ...
      En_suspt_st*Efactor*A1*4*pi*(D/2)^2
    }
